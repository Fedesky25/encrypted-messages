...
class OutBox extends HTMLElement {
    private readonly textual: HTMLDivElement;
    private text: string = '';
    ...
    connectedCallback() {
        if(this.getAttribute("break") != null) this.textual.classList.add("break");
    }
    setText(text: string) {
...
document.getElementById("start-receiving").addEventListener("click", async function() {
    ...
    const decrypted = (document.getElementById("decrypted") as OutBox);
    const form = document.getElementById("form-receiver");
    const input = form.querySelector("input");
    form.addEventListener("submit", async function(ev) {
        ev.preventDefault();
        if(!private) return void (input.value = '');
        try {
    ...
    (document.getElementById("gen-key") as OutBox).setText(arrayBufferToBase64(buffer));
...
(function(){
    ...
    const encrypted = (sender.querySelector("out-box") as OutBox);
...
function adaptElementHeight(el: HTMLElement) {
    console.log("adapting");
    el.style.height = "auto";
    el.style.height = el.scrollHeight + "px";
}

interface ThrottleEventData<E extends HTMLElement> {
    element: E;
    request: number;
    callback: (el: E) => any;
}
function throttleEvent<E extends HTMLElement>(element: E, event: string, ms: number, fn: (el: E) => any) {
    const data: ThrottleEventData<E> = {element, request: null, callback: fn}
    element.addEventListener(event, function() {
        if(!data.request) data.request = setTimeout(throttleEventExec, ms, data);
    });
}
function throttleEventExec(data: ThrottleEventData<HTMLElement>) {
